...
// RequireMissing fails if the dotted JSON path exists on the response body.
func (r Result) RequireMissing(path string) Result {
	r.t.Helper()
	_, err := lookupJSON(unmarshalJSONValue(r.t, r.Body), path)
	require.Error(r.t, err, "path %s should be absent in %s", path, r.String())
	return r
}
