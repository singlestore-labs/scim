...
func (h *TestClient) GetUser(id string, opts ...ListOpts) Result {
	var query url.Values
	if len(opts) > 0 {
		query = listQuery(opts[0])
	}
	return h.do(http.MethodGet, "/Users/"+url.PathEscape(id), query, nil)
}
...
func (h *TestClient) GetGroup(id string, opts ...ListOpts) Result {
	var query url.Values
	if len(opts) > 0 {
		query = listQuery(opts[0])
	}
	return h.do(http.MethodGet, "/Groups/"+url.PathEscape(id), query, nil)
}
